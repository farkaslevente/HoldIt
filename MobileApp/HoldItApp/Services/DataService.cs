...
namespace MobilApp_Szakdolgozat.Services
{
    public class DataService
    {
        ...
        static string url202 = "http://10.0.22.5:9000";        
        public static string url = url202; 					//Your IP address and PORT number must be here like the example one row above
        ...
        public static async Task<IEnumerable<PictureModel>> getAllPictures()
        {
            using (var client = new HttpClient())
            {
                string token = await SecureStorage.GetAsync("userToken");
                ...
                return JsonConvert.DeserializeObject<List<PictureModel>>(result);
            }
        }

        public static async Task<IEnumerable<PostModel>> getAds()
        {
            using (var client = new HttpClient())
            {
                string token = await SecureStorage.GetAsync("userToken");
        ...
        {
            posts.Clear();
            IEnumerable<PostModel> list = await getAds();
